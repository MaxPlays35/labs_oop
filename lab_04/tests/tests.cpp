//
// Created by MaxPlays on 29/10/2024.
//
...
TEST(TRIANGLE_CONSTRUCTORS_TESTS, CENTER) {
    ...
    const auto correct_points = std::vector<Shape::Point<double>>{
        Shape::Point{
...
TEST(TRIANGLE_SQUARE_TESTS, SQUARE) {
    const auto center = Shape::Point<double>{0, 0};
    const double length = 5.0;
    auto triangle = Shape::Triangle(center, length);
...
TEST(ARRAY_TESTS, TOTAL_SQUARE) {
    auto array = Shape::Array<Shape::Figure<double>*, double>();

    auto figure1 = Shape::Triangle<double>({0.0, 0.0}, 5.0);
    auto figure2 = Shape::Hexagon<double>({1.0, 1.0 / std::tan(std::numbers::pi / 6.0)}, 2.0);
    auto figure3 = Shape::Octagon<double>({1.0, 1.0 / std::tan(std::numbers::pi / 8.0)}, 2.0);
...
TEST(ARRAY_TESTS, TOTAL_SQUARE_ONLY_TRIANGLES) {
    auto array = Shape::Array<Shape::Triangle<double>, double>();

    auto figure1 = Shape::Triangle<double>({0.0, 0.0}, 5.0);
    auto figure2 = Shape::Triangle<double>({1.0, 1.0 / std::tan(std::numbers::pi / 6.0)}, 2.0);
    auto figure3 = Shape::Triangle<double>({1.0, 1.0 / std::tan(std::numbers::pi / 8.0)}, 2.0);
...
TEST(ARRAY_TESTS, TOTAL_SQUARE_ONLY_HEXAGONS) {
    auto array = Shape::Array<Shape::Hexagon<double>, double>();

    auto figure1 = Shape::Hexagon<double>({0.0, 0.0}, 5.0);
    auto figure2 = Shape::Hexagon<double>({1.0, 1.0 / std::tan(std::numbers::pi / 6.0)}, 2.0);
    auto figure3 = Shape::Hexagon<double>({1.0, 1.0 / std::tan(std::numbers::pi / 8.0)}, 2.0);
...
TEST(ARRAY_TESTS, TOTAL_SQUARE_ONLY_OCTAGONS) {
    auto array = Shape::Array<Shape::Octagon<double>, double>();

    auto figure1 = Shape::Octagon<double>({0.0, 0.0}, 5.0);
    auto figure2 = Shape::Octagon<double>({1.0, 1.0 / std::tan(std::numbers::pi / 6.0)}, 2.0);
    auto figure3 = Shape::Octagon<double>({1.0, 1.0 / std::tan(std::numbers::pi / 8.0)}, 2.0);
